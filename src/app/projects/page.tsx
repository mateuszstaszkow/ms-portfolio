...
const Projects: React.FC = () => {
  ...
  const getLastTimestamp = (project: Project): number => {
    ...
    return mapToDate(dates.at(-1)).getTime();
  }

  const getMonthsDifference = (project): number => {
    const dates: string[] = project.dates.split(' ');
    if (dates.length === 1) {
      return '';
    }
    const firstDate = mapToDate(dates[0]);
    const lastDate = mapToDate(dates.at(-1), 28);
  ...
  const projects: Project[] = [...PROJECTS].reverse()
    ...
    .map(project => ({ ...project, monthsDifference: getMonthsDifference(project) }));

  return (
    <div>
      <SubPageHeader id="projects" title="Projects" label={`${projects.length} commercial & private projects`}>
      </SubPageHeader>
